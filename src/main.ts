...
let items = { undo: 2, shuffle: 2, push: 2 };
const emojiTypes = ["🐰", "🍄", "🐢", "🦊", "🐸", "🐼", "🐵", "🦉", "🐱", "🐶", "🦄", "🐝", "🍓", "🍉"];
...
function initStage() {
    ...
    // Stage 1 starts with 8 triplets (24 tiles). Increases gradually.
    let triplesCount = 8 + Math.floor((currentStage - 1) * 2);
    let totalTiles = triplesCount * 3;
    let allowedTypes = Math.min(5 + Math.floor((currentStage-1) / 3), emojiTypes.length);
    
    let typesPool = [];
    for(let i=0; i<triplesCount; i++) {
        let t = emojiTypes[i % allowedTypes];
        typesPool.push(t, t, t);
    }
    typesPool.sort(() => Math.random() - 0.5);
    
    let idCounter = 0;
    let maxLayers = Math.min(6, 2 + Math.floor(currentStage/3));
    
    while(typesPool.length > 0) {
        let z = Math.floor(Math.random() * maxLayers);
        
        let minXY = 2 + z; 
        let maxXY = 10 - z;
        if(maxXY < minXY) maxXY = minXY;
        
        let x = Math.floor(Math.random() * (maxXY - minXY + 1)) * 2 + minXY;
        let y = Math.floor(Math.random() * (maxXY - minXY + 1)) * 2 + minXY;
        
        if(Math.random() > 0.6) { x += 1; y += 1; }
        
        if(tiles.some(t => t.x === x && t.y === y && t.z === z)) continue;
        
        tiles.push({ id: idCounter++, type: typesPool.pop(), x: x, y: y, z: z, inTray: false, inStash: false });
    }
...
function checkOverlaps() {
    tiles.forEach(t1 => {
        ...
        for(let t2 of tiles) {
            ...
            if(t2.z > t1.z) {
                let overlapX = (t1.x < t2.x + 2) && (t1.x + 2 > t2.x);
                let overlapY = (t1.y < t2.y + 2) && (t1.y + 2 > t2.y);
...
function checkMatches() {
    ...
    if(matchedType) {
        ...
        clearInterval(timerInterval);
        setTimeout(() => { alert("Game Over! Tray is full.\nTip: Use Push Out to make space!"); currentStage=1; initStage(); }, 100);
    } else {
        checkWin();
    }
}
...
document.getElementById("btn-undo").onclick = () => {
    ...
    t.element.style.top = (80 + t.y * GRID_CELL) + "px";
